...
def main(args):
    ...
    logger = utils.get_log(
        "./log/" + save_str + f'-{datetime.now().strftime("%Y-%m-%d %H-%M-%S")}.txt'
    )
    ...
    # 首先创建数据集划分（如果不存在）
    root = args.root_local if eval(str(args.local)) else args.root_sever
    split_file = retrieval_datasets.get_split_file_name(
        sketch_info["format"], args.pair_mode, args.task
    )
    ...
    # 【已修改】将 args.is_create_fix_data_file 修正为 args.create_fix_data_file
    if eval(str(args.create_fix_data_file)) or not os.path.exists(split_file):
        ...
        retrieval_datasets.create_dataset_split_file(
            ...
            # 【已修改】将 args.is_full_train 修正为 args.full_train
            full_train=eval(str(args.full_train)),
        )

    # 创建数据加载器
    train_set, test_set, train_loader, test_loader, dataset_info = (
        retrieval_datasets.create_sketch_image_dataloaders(
            batch_size=args.bs,
    ...
    model = sbir_model_wrapper.create_sbir_model_wrapper(
        ...
        # 【已修改】将 args.is_freeze_image_encoder 修正为 args.freeze_image_encoder
        freeze_image_encoder=eval(str(args.freeze_image_encoder)),
        # 【已修改】将 args.is_freeze_sketch_backbone 修正为 args.freeze_sketch_backbone
        freeze_sketch_backbone=eval(str(args.freeze_sketch_backbone)),
    ...
    # 恢复训练（如果指定）
    # 【已修改】将 args.is_load_ckpt 修正为 args.load_ckpt
    if eval(str(args.load_ckpt)):
        model_trainer.load_checkpoint(check_point)
    else:
        print("不加载权重，从零开始训练模型")

    # 开始训练
    # 【已修改】将 args.is_vis 修正为 args.vis
    if eval(str(args.vis)):

...
def _load_pretrained_weights(model_name):
    """
    缓存预训练权重，避免重复下载和加载
    【已修改】此函数现在从本地文件加载权重，而不是联网。
    """
    print(f"首次加载预训练模型权重：{model_name}")

    # ======================= 开始修改 1 =======================
    # 我们不再直接调用 pretrained=True，而是分步从本地加载

    # 1. 定义本地权重文件的路径
    local_weights_path = "./pytorch_model.bin"

    try:
        print(f"-----> [Optimized] 正在从本地路径加载权重: {local_weights_path}")
        # 2. 直接从本地文件加载权重字典
...
class CachedVisionModel(nn.Module):
    ...
    def __init__(self, model_name="vit_base_patch16_224"):
        ...
        # ======================= 开始修改 2 =======================
        if cached_weights:
            # 使用 strict=False 来加载权重，忽略不匹配的键（如分类头）
            self.model.load_state_dict(cached_weights, strict=False)
        else:
            print(f"!!!!!! [WARNING] 未能为 {model_name} 加载任何预训练权重。")
        # ======================= 结束修改 2 =======================
...
# create_optimized_vision_model 函数及之后的所有代码保持您原始版本的原样，无需任何修改
def create_optimized_vision_model(
    model_name="vit_base_patch16_224", checkpoint_path=None
):
    """
    创建优化的确定性视觉模型
...
def test_optimized_model():
    ...
    print(
        f"加速比: {first_load_time/second_load_time if second_load_time > 0 else float('inf'):.1f}x"
    )
